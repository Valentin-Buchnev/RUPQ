        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                iou, box_filtered_index = bbox_iou(
                    pred_box.unsqueeze(0), torch.stack(filtered_targets).detach().cpu()
                ).max(0)
...
class MAP(torchmetrics.Metric):
    """
    Class calculating mAP metric.
    Code is taken from https://github.com/eriklindernoren/PyTorch-YOLOv3/tree/master.
    """

    def __init__(self, img_size, iou_thres, conf_thres, nms_thres, **kwargs):
        ...
        self.class_names = load_classes(DATASETS_INFO["coco"]["names"])
        self.reset()
    ...
    def update(self, model, batch):
        ...
        with torch.no_grad():
            outputs = model(imgs)
            outputs = non_max_suppression(outputs, conf_thres=self.conf_thres, iou_thres=self.nms_thres)

        self.sample_metrics += get_batch_statistics(outputs, targets, iou_threshold=self.iou_thres)

    def compute(self):
        true_positives, pred_scores, pred_labels = [np.concatenate(x, 0) for x in list(zip(*self.sample_metrics))]
        metrics_output = ap_per_class(true_positives, pred_scores, pred_labels, self.labels)
