...
class Upsampler(nn.Sequential):
    def __init__(self, scale, n_features):
        ...
        if (scale & (scale - 1)) == 0:  # check if scale == 2^n
            ...
            modules.append(nn.Conv2d(n_features, 9 * n_features, kernel_size=3, padding=1, bias=True))
...
class EDSR(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, feature_channels=64, n_residual_blocks=16, scale=2):
